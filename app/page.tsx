  ...
  return (
    <main className="min-h-screen bg-black text-white font-sans">
      <section className="relative flex min-h-[90vh] flex-col items-center justify-center p-6 text-center overflow-hidden">
        ...
        <div className="relative z-10 space-y-6 max-w-5xl">
          ...
          <p className="max-w-2xl mx-auto text-gray-400 text-lg md:text-xl font-medium">
            Verilerinizi blokzincir gücüyle fütüristik grafiklere dönüştüren veri görselleştirme terminali.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-10">
            <Link href="/auth/login" className="w-full sm:w-auto px-12 py-5 bg-yellow-500 text-black font-black uppercase text-xs rounded-2xl transition-all hover:scale-105 active:scale-95">
              Sisteme Giriş Yap
            </Link>
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="p-8 rounded-[40px] border border-white/5 bg-[#050505]">
          <BarChart3 className="text-yellow-500 mb-6" size={24} />
          <h3 className="text-xl font-black uppercase italic">Snap-Designer</h3>
          <p className="text-sm text-gray-500 mt-4 italic">Excel entegrasyonuyla anlık fütüristik grafikler.</p>
        </div>
        <div className="p-8 rounded-[40px] border border-white/5 bg-[#050505]">
          <CreditCard className="text-yellow-500 mb-6" size={24} />
          <h3 className="text-xl font-black uppercase italic">Pi-Pay Billing</h3>
          <p className="text-sm text-gray-500 mt-4 italic">Pi Mainnet üzerinden saniyeler içinde kota alımı.</p>
        </div>
        <div className="p-8 rounded-[40px] border border-white/5 bg-[#050505]">
          <Globe className="text-yellow-500 mb-6" size={24} />
          <h3 className="text-xl font-black uppercase italic">Universal Embed</h3>
          <p className="text-sm text-gray-500 mt-4 italic">Web sitenize tek satır kodla grafik yerleştirin.</p>
        </div>
      </section>

      <footer className="py-10 text-center text-[10px] text-gray-700 uppercase tracking-[0.5em] font-bold">
        © 2026 SnapLogic • Built for Pi Network
      </footer>
    </main>
  );
}
