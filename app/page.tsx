...
export default function LandingPage() {
  ...
  const handleStart = async () => {
    // Window kontrolü ve Pi SDK doğrulaması
    if (typeof window !== "undefined" && window.Pi) {
      try {
        ...
        // Başarılı girişte terminale yönlendir
        window.location.href = "/auth"; 
      } catch (err) {
        console.error("Pi Auth Hatası:", err);
        alert("Giriş yapılamadı, lütfen Pi Browser kullanın.");
      }
    } else {
      alert("Pi SDK hazır değil veya Pi Browser dışındasınız!");
    }
  };
    ...
    <div className="min-h-screen bg-black text-white overflow-hidden">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-8 pt-20 pb-32 flex flex-col items-center text-center">
        ...
        <p className="max-w-2xl text-gray-500 text-sm md:text-lg font-medium mb-12 leading-relaxed">
          SnapScript v0 ile verilerinize ruh üfleyin. Dünyanın ilk Pi-Native veri görselleştirme motoruyla tanışın.
        </p>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-3 gap-8 pb-20">
        ...
        <div className="p-8 rounded-[40px] bg-white/5 border border-white/10">
          ...
          <p className="text-xs text-gray-500 font-bold leading-relaxed">İşlemleriniz Pi blokzinciri ile mühürlenir.</p>
        </div>
      </section>
    </div>
  );
