<template>
  <div class="page">
    <div class="head">
      <h1>Gig Services</h1>
      <p>Freelance services. No escrow. Direct contact.</p>
      <div class="row">
        <input v-model="q" class="input" placeholder="Search gigs…" />
        <button class="btn" @click="open=true">+ Add Gig</button>
      </div>
    </div>

    <div class="grid">
      <div v-for="g in filtered" :key="g.id" class="card">
        <div class="tag">{{ g.tag }}</div>
        <h3>{{ g.title }}</h3>
        <p class="muted">by {{ g.seller }}</p>
        <p class="desc">{{ g.desc }}</p>
        <div class="actions">
          <button class="btn2" @click="hire(g)">Hire (demo)</button>
    ...
    <div v-if="open" class="overlay" @click.self="open=false">
      <div class="modal">
        <h2>Add Gig</h2>
        <div class="form">
          <input v-model="form.title" class="input" placeholder="Gig title" />
          <input v-model="form.seller" class="input" placeholder="Seller name" />
          <input v-model="form.tag" class="input" placeholder="Tag (Design, Dev, Video…)" />
...
const form = reactive({ title: "", seller: "", tag: "", desc: "" })

const items = ref([
  { id: 1, title: "Logo Design (Minimal)", seller: "AbsürdDünyalar Studio", tag: "Design", desc: "Clean logo + brand kit." },
  { id: 2, title: "Landing Page (Vue)", seller: "Onur Dev", tag: "Development", desc: "Fast Vite + Vue page." },
  { id: 3, title: "YouTube Shorts Editing", seller: "SamataTV", tag: "Video", desc: "Shorts cut + captions." },
])

const filtered = computed(() => {
  const s = q.value.trim().toLowerCase()
  if (!s) return items.value
  return items.value.filter(x => (x.title + " " + x.seller + " " + x.tag + " " + x.desc).toLowerCase().includes(s))
})

function add() {
  if (!form.title) return alert("Title required")
  items.value.unshift({ id: Date.now(), title: form.title, seller: form.seller || "—", tag: form.tag || "—", desc: form.desc || "—" })
  form.title = form.seller = form.tag = form.desc = ""
  open.value = false
}
function hire(g) { alert(`Hire: ${g.title} (demo)`) }
